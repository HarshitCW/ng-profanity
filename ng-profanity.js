var app = angular.module('ngProfanity', []);
app.provider('swear', function(){
    ...
    var swearwords = ['asshole', 'ass', 'fucker', 'fucked', 'fucking', 'fuck', 'bitches', 'bitching', 'bitched', 'bitch', 'bastard', 'damn', 'shitting', 'shitty', 'shit', 'cock'];

    var swearwordRegStr = swearwords[0];
    for(var i = 1; i < swearwords.length; i++)
        swearwordRegStr += '|' + swearwords[i];
    this.$get = function(){
        return new RegExp(swearwordRegStr, 'gi');
    }
});

app.service('Profanity', ['swear', ProfanityService]);
app.filter('profanity', ['swear', ProfanityFilter]);

function ProfanityService(swear){
    function containsProfanity(input){
        return swear.test(input);
    }
...
function ProfanityFilter(swear){
    return function(input, enable){
        if(enable && input){
            input = input.replace(swear, function(x){
                return x.replace(/./g, '*');
            });
        }
        return input;
